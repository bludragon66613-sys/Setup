...
## Google Access (Browser-Based)
GOG is deferred. Use the \rowser\ tool for Gmail/Calendar/Sheets access:
1. \rowser action=open targetUrl=https://gmail.com\
2. \rowser action=snapshot\ — read the UI
3. \rowser action=act kind=click ref=<ref>\ — interact
4. For structured data, prefer \web_fetch\ with Google API endpoints if you have an API key.

## Google API Key
- Stored in OpenClaw auth: \google:aistudio\ (free tier)
- Use for Gemini/Vertex calls, not Gmail/Calendar CRUD.
