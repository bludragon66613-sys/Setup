...
2. **n8n** — Workflow automation on :5678 (400+ integrations, MCP-managed)
3. **Paperclip** — Agent platform on :3100 (start before dashboard)
4. **Dashboard** — NERV on :5555 (`--webpack` mode, NOT turbopack — Windows bug)
...
Local n8n instance for multi-service workflow automation, managed via n8n-mcp.
- **URL**: http://localhost:5678
- **Version**: 2.15.0
- **Data**: `~/.n8n/` (SQLite)
- **MCP**: `n8n-mcp` in `.mcp.json` — search 1,396 nodes, 2,646 templates, full CRUD
- **API key**: Set in `~/.claude/.mcp.json` after first-time browser setup
- **Use for**: Service integrations, email pipelines, crypto alerts, webhook triggers, data pipelines
