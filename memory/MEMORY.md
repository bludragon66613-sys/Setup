...
## Session Savepoints (latest 3 — 48 older ones in _archive/)
- [session_savepoint_2026-04-28e_drip-decode-navsession-vercel.md](session_savepoint_2026-04-28e_drip-decode-navsession-vercel.md) — drip /decode/[name] + NavSession + Vercel build chain + auth/orcid/link + PATCH audit. 9 commits on phase-0-novelty-gateway pushed. Shipped 3 backlog items (decode detail w/ generateStaticParams, NavSession header consumer, eval daily-metrics +3 fields). Unblocked persistent Vercel: FAILURE by porting */5 cron from Vercel (Hobby caps at daily) to Inngest free tier AND fixing Turbopack-only build error in @drip/auth (.js suffix imports unresolvable). Closed audit-tail w/ active-verb capability regex. Shipped /api/auth/orcid/link working route (mock mode end-to-end). Closed PATCH_WEBSITE_CONTENT.md status row + §4 cross-doc audit (items 1, 5, 8 closed; 2, 3, 6 deferred to avoid Matteo-sync conflict; 4 = automation-stack rewrite tracked for next session). Browser-verified /decode + /decode/[name] + NavSession + /api/auth/* on both localhost AND Vercel preview — parity confirmed, no prod-only regressions. Branch 85 → 93 ahead. **277 tests green** (was 261, +16). PR #1: mergeable=CLEAN, all 8 checks SUCCESS (was UNSTABLE/Vercel-FAILURE all session before fix landed). REQUIRES Rohan: set DRIP_WEB_BASE_URL on apps/api Vercel env + mirror CRON_SECRET. 3 leaked secrets still need rotation (carryover).
- [session_savepoint_2026-04-28d_drip-license-sweep-and-auth.md](session_savepoint_2026-04-28d_drip-license-sweep-and-auth.md) — drip license sweep close + /api/auth/* mock layer. 8 more commits on phase-0-novelty-gateway, pushed to origin. Closed the license-conformance audit-tail (PIPELINE Aurora preface, V2/V_CLARITY/BRIEF/CHECKPOINT reframes, IP-NFT clarification, docs-scope CI guard at apps/web/lib/__tests__/license-copy-docs.test.ts with 4 regexes + whole-file allowlist). Mounted @drip/auth on /api/auth/* (5 routes — login, callback, me, logout, orcid/verify — 22 vitest cases, mock-first; DRIP_AUTH_REAL=1 swaps to stub real client). Hardened .gitignore with .env.* glob plus !.env.example exception. 261 tests green (was 238). Branch 75 → 85 ahead. PR #1 MERGEABLE, CI re-running. **Three secrets flagged for rotation**: Neon DATABASE_URL, MoonPay webhook secret, Helio bearer token (all leaked into transcript jsonl this session via .env.vercel.prod read + docs/PENDING.md read; PENDING.md deleted, .env.* now gitignored).
- [session_savepoint_2026-04-28c_drip-license-followups.md](session_savepoint_2026-04-28c_drip-license-followups.md) — drip license followups + Auth0/ORCID scaffold. 7 more commits on phase-0-novelty-gateway: legal/creator-tos.md mirror, license-copy CI guard test (auto-fails reintroduction), /pipeline page reframe, DRIP-V2 §7.0 IP licensing structure, BUILDERS.md license preface mirror, V1 supersession banner, waitlist tier values fix (form + route + Section 05 copy all aligned with run-based pricing), @drip/auth mock-first scaffold (10th workspace, 24 tests, ResearcherTier locked, deterministic mock + stub real). 238 tests green. PR #1 description updated. Telegram reply ready to send. Branch 67 → 75 ahead.
