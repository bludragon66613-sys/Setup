...
Windows-patched install at `~/Developer/browser-harness`. Invoke as `browser-harness` (on PATH). Use for tasks needing logged-in Chrome (GitHub, X, LinkedIn, drip.markets). Prefer `/browse` from gstack for stateless fetches. **Read `~/Developer/browser-harness/SKILL.md` only when actually using harness.**