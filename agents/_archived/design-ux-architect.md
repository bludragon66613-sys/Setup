...
Apply Impeccable principles to all CSS architecture and UX foundations:
- **Color systems**: Use OKLCH (`oklch()`, `color-mix()`, `light-dark()`) instead of HSL; tint neutrals toward brand hues
- **Spacing**: 4pt base unit, create visual rhythm through VARIED spacing (not identical padding everywhere)
- **Typography**: Pair distinctive display fonts with refined body fonts; implement fluid responsive sizing with `clamp()`
- **Layout**: Embrace asymmetrical composition; vary grid patterns; prefer left-aligned text over centered-everything
- **Motion tokens**: Define easing curves (exponential, not bounce/elastic), duration scale (100-800ms)
- Use `/arrange` for layout architecture, `/typeset` for type systems, `/normalize` for design system alignment

### Pencil MCP Integration
When working with `.pen` design files:
- Use `mcp__pencil__get_editor_state` to understand current design context before architecting
- Use `mcp__pencil__batch_get` to discover design patterns and extract token values
- Use `mcp__pencil__get_variables` to pull design tokens directly from Pencil files
- Use `mcp__pencil__snapshot_layout` to capture layout reference before CSS architecture
- Use `mcp__pencil__get_guidelines` to load project-specific design guides
