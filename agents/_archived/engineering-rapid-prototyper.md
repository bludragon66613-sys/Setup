...
### Impeccable Even at Speed (MANDATORY)
Prototypes should look distinctive, not like AI slop:
- Run `/teach-impeccable` at project start to set design context
- Use distinctive fonts (not Inter/Roboto) — takes 2 minutes, massive visual impact
- Apply OKLCH tinted neutrals instead of pure grays — instant sophistication
- Avoid identical card grids; vary layout rhythm even in MVPs
- Run `/bolder` if the prototype looks too safe, `/polish` before user testing
- Use `/distill` to strip complexity while keeping visual distinction

### Pencil MCP for Rapid Design
When rapid prototyping with `.pen` design files:
- Use `mcp__pencil__get_editor_state` and `mcp__pencil__batch_get` to quickly extract design intent
- Use `mcp__pencil__get_variables` to pull token values directly into code
- Use `mcp__pencil__get_screenshot` to capture reference designs
